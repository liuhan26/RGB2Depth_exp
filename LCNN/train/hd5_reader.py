...
class hd5_reader():
    ...
    def train_nextbatch(self):
        ...
        if self.read_data_flag == 1:
            ...
            train_data_batch = self.train_data[
                               (self.train_data_iter - 1) * self.train_bsize:self.train_data_iter * self.train_bsize]
            train_label_batch = self.train_label[
                                (self.train_data_iter - 1) * self.train_bsize:self.train_data_iter * self.train_bsize]
    ...
    def val_nextbatch(self):
        ...
        if self.val_data_iter == self.val_data_ITERS:
            ...
            val_data_batch = self.val_data[
                             (self.val_data_iter - 1) * self.val_bsize:self.val_data_iter * self.val_bsize]
            val_label_batch = self.val_label[
                              (self.val_data_iter - 1) * self.val_bsize:self.val_data_iter * self.val_bsize]
